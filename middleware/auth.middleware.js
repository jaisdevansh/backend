...
export const protect = async (req, res, next) => {
    try {
        ...
        const CACHE_KEY = `auth_status_${decoded.id}`;
        ...
        if (!cached) {
            const projection = 'role isActive';
             // Atomic, fast lookup
            const user = await User.findById(decoded.id).select(projection).lean() ||
                   await Host.findById(decoded.id).select(projection).lean() ||
                   await Staff.findById(decoded.id).select(projection).lean() ||
                   await Admin.findById(decoded.id).select(projection).lean();
