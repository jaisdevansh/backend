...
export const createHost = async (req, res, next) => {
    try {
        ...
        await cacheService.flushAll(); // For host creation, flush all to ensure pending/lists are fresh
...
export const getPendingHosts = async (req, res, next) => {
    try {
        ...
        const [pendingHosts, count] = await Promise.all([
            Host.find({ hostStatus: 'PENDING_VERIFICATION' })
                .select('name email phone kyc profileImage createdAt hostStatus')
                .skip(skip)
                .limit(limit)
                .sort({ createdAt: -1 })
                .lean(),
            Host.countDocuments({ hostStatus: 'PENDING_VERIFICATION' })
        ]);
...
export const verifyHost = async (req, res, next) => {
    try {
        ...
        await cacheService.delete('admin_dashboard_stats');
        await cacheService.delete('pending_hosts'); // If you cache this pattern
        await cacheService.flushAll(); // Ensure consistency across all admin tools
...
export const getAllStaff = async (req, res, next) => {
    try {
        ...
        const CACHE_KEY = `admin_staff_p${page}_t${typeFilter || 'ALL'}`;
        const cached = await cacheService.get(CACHE_KEY);
        if (cached) return res.status(200).json({ ...cached, source: 'cache_hit' });

        const query = typeFilter ? { staffType: typeFilter } : {};

        const [staff, total] = await Promise.all([
            Staff.find(query)
                .select('name email phone staffType isActive createdAt')
                .skip(skip)
                .limit(limit)
                .sort({ createdAt: -1 })
                .lean(),
...
export const getAdminStats = async (req, res, next) => {
    try {
        ...
        const [userCount, activeHosts, totalHosts, pendingHosts, totalBookings, revenueAgg] = await Promise.all([
            ...
            Host.countDocuments({ hostStatus: { $in: ['CREATED', 'PENDING_VERIFICATION'] } }),
...
export const toggleUserStatus = async (req, res, next) => {
    try {
        ...
        await cacheService.flushAll(); // High-priority status change requires fleet-wide invalidation
